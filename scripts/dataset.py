...
class ZarrDataset(data.Dataset):
    ...
    def __init__(self, root_path=None, img_id=None, tile_size=None, overlap=None, threshold=None, mask_id='default',
                 transform=None, label=None):
    ...
    def load_tiling_mask(self, mask_path):
        ...
        if mask_path is not None:  # Load mask from existing file
            ...
            assert abs(mask_temp.shape[1] / mask_temp.shape[
                0] - slide_width / slide_height) < 0.01, 'Mask shape does not match slide shape'
        else:
...
class ZarrSlidesDataset(data.Dataset):
    ...
    def __init__(self, slides_root_path=None, img_ids=None, tile_size=None, overlap=None, threshold=None,
                 mask_id='default', transform=None, labels=None, dataset_class=None):
    ...
    def get_slides(self, img_ids, labels, dataset_class):
        ...
        for img_id, img_label in tqdm(zip(img_ids, labels)):
            ...
            slide = dataset_class(self.slides_root_path, img_id, self.tile_size, self.overlap, self.threshold,
                                  self.mask_id, self.transform, img_label)
